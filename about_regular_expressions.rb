...
class AboutRegularExpressions < EdgeCase::Koan
  ...
  ## They are a 'blackhole' for matches. They take them all in, not just
  ...
  # remember that "z" has the * which means zero or more
  # contrast with [/a*z/]
  ...
  def test_slash_d_is_a_shortcut_for_a_digit_character_class
    assert_equal "42", "the number is 42"[/[0123456789]+/] # + is 
  ...
  def test_shortcut_character_classes_are_negated_with_capitals
    assert_equal "the number is ", "the number is 42"[/\D+/] #any non-digit (until digit)
    assert_equal "space:", "space: \t\n"[/\S+/] #non-whitespace characters until one
  ...
  def test_slash_a_anchors_to_the_start_of_the_string
    ...
    assert_equal nil, "start end"[/\Aend/] #rubular doesn't work like this
  end
  ...
  def test_dollar_sign_anchors_to_the_end_of_lines
    assert_equal "42", "2 lines\nnum 42"[/\d+$/] #end, number onward to non-num
  end
  ...
  def test_variables_can_also_be_used_to_access_captures
    assert_equal "Gray, James", "Name:  Gray, James"[/(\w+), (\w+)/] # why not name?
    assert_equal "Gray", $1
